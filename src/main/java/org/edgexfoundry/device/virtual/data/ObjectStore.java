...
public class ObjectStore {
	...
	public void put(Device device, ResourceOperation operation, String value) {
		...
		List<VirtualObject> objectsList = createObjectsList(operation, device);

		String deviceId = device.getId();
		List<Reading> readings = new ArrayList<>();

		for (VirtualObject obj : objectsList) {
	...
	private List<VirtualObject> createObjectsList(ResourceOperation operation, Device device) {
		Map<String, VirtualObject> objects = profiles.getObjects().get(device.getName());
		List<VirtualObject> objectsList = new ArrayList<VirtualObject>();
		if (operation != null && objects != null) {
			VirtualObject object = objects.get(operation.getObject());
	...
	private String transformResult(String result, VirtualObject object, Device device,
